...
router.get(
    "/config/esporte-horarios/:esporteId",
    authMiddleware,
    requireAdmin,
    async (req, res) => {
...
router.post(
    ...
    async (req, res) => {
        ...
        try {
            if (isPadrao) {
                ...
                if (jaTemPadrao) {
                    return res.status(409).json({
                        erro:
                            "Já existe uma regra padrão (diaSemana = null) para esse esporte e tipo de sessão.",
                    });
                }
            }
            ...
            // audit (assinatura de 1 argumento)
            try {
                await logAudit({
                    req,
            ...
            if (String(err?.code) === "P2002") {
                return res.status(409).json({
                    erro:
                        "Config já existe para esse esporte/dia/tipo. Edite a existente ou remova antes.",
                });
            }
    ...
    async (req, res) => {
        ...
        try {
            ...
            if (String(err?.code) === "P2002") {
                return res.status(409).json({
                    erro:
                        "Conflito de unicidade: esporte/dia/tipo já existe. Ajuste a combinação.",
                });
            }
