...
export default function AdminMasterDashboardPage() {
  ...
  // ✅ NOVO: média de logins
  const [mediaLogins, setMediaLogins] = useState<string>("__loading__");
    ...
    const carregarTudo = async () => {
      try {
        const [
          ...
          // ✅ NOVO ENDPOINT (ajuste se o prefixo do router for outro)
          axios.get<LoginsResumoResp>(`${API_URL}/login/estatisticas/logins/resumo`, { withCredentials: true }),
        ]);
        ...
        const mediaLg = Number(loginsResp.data?.mediaPorDia ?? 0);
        setMediaLogins(String(Math.round(mediaLg)));
      } catch (err) {
        ...
        setMediaLogins((v) => (v === "__loading__" ? "—" : v));
      }
    };
  ...
  const resumoLinha1 = useMemo(() => {
    return resumoLinha1Base.map((c) => {
      ...
      if (c.label === "Média logins") return { ...c, value: mediaLogins };
      return c;
    });
  }, [totalUsuarios, mediaAgendamento, totalEsportes, totalQuadras, totalChurrasqueiras, totalProfessores, mediaLogins]);
